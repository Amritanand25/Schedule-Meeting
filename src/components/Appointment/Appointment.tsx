...
const Appointment: React.FC = () => {
  ...
  const changeDateHandler = (value: any) => {
    ...
    let dur = 0;
    timeArray.map((item) => {
      if (item.isActive) dur = item.time;
      return item;
    });
  ...
  return (
    <div className="w-[75%]">
      <Navigation />
      <div className="text-center text-2xl font-bold text-blue my-12">
        <h2>Choose Date & Time</h2>
      </div>
      <div className="flex justify-center items-center">
        <Calendar value={date} onChange={changeDateHandler} />
        {(!error || (suggestedData && suggestedData.length > 0)) && (
      ...
      <div className="flex p-4 mt-8 justify-center items-center">
        ...
        {(!showNextBtn || error) && (
          <button className="text-white text-md font-medium bg-blue bg-opacity-40 px-6 py-3 flex items-center rounded-md cursor-default">
            NEXT STEP
            <span className="ml-4 font-extrabold text-xl">
              <MdOutlineNavigateNext />
            </span>
